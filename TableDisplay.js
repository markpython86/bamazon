...
var displayTable = function() {
    ...
    this.displayInventoryTable = function(response) {
	    ...
	    for (var index=0; index <this.response.length; index++) {
	        this.table.push(
	            [this.response[index].item_id, this.response[index].product_name, this.response[index].department_name, '$'+ this.response[index].price, this.response[index].stock_quantity] );
	    }
    	console.log('\n' + this.table);
	};
}
